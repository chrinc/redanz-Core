...
public class VolunteeringService {
  ...
  public void updateVolunteering(JsonObject request, Event event) throws IOException, TemplateException {
    Set<EventSlot> newVolunteerSlots = new HashSet<>();
    List<VolunteerType> newVolunteerTypes = new ArrayList<>();

    getVolunteerSchema(event).forEach(
      stringStringMap -> {
                ...
                switch(key) {
                  case "volunteerSlots":
                    request.get(key).getAsJsonArray().forEach(item -> {
                      newVolunteerSlots.add(slotService.findByEventSlotId(item.getAsLong()));
                    });
                    break;
                  case "volunteerTypes":
                    request.get(key).getAsJsonArray().forEach(item -> {
...
//    List<EventSlot> existingVolunteerSlots
//      = slotService.getAllEventSlots(SlotType.VOLUNTEER, event);

    // Save new volunteering slots
//    newVolunteerSlots.stream()
//      .filter(volunteerSlot -> !existingVolunteerSlots.contains(volunteerSlot))
//      .forEach(volunteerSlot -> slotService.save(
//          new EventSlot(, event, volunteerSlot.getSeqNr()
//          )
//        )
//      );

    // Remove existing volunteering slots not in new volunteering slots
...
//        EventSlot eventSlot = eventTypeSlotService.findByEventAndTypeSlot(event, slotService.findByTypeAndSlot("volunteer", existingHostingSlot));
...
//    event.setVolunteerTypes(volunteerTypes);
    eventRepo.save(event);
//    slotService.delete(event, SlotType.VOLUNTEER);
  }

  public List<Map<String, String>> getVolunteerSchema(Event event) {
  ...
  public List<Map<String, String>> getVolunteerEventSlots(Event event) {
    return
      slotService.getAllEventSlots(SlotType.VOLUNTEER, event)
      .stream()
      .map(eventSlot -> eventSlot.dataMap())
  ...
  public List<EventSlot> eventVolunteerList (Event event) {
    return slotService.getAllEventSlots(SlotType.VOLUNTEER, event);
  }
    ...
    return new HashMap<>() {
      {
        ...
        put("volunteerSlots", slotService.getAllEventSlots(SlotType.VOLUNTEER, event).toString());
